                    ...
                    @foreach ($permissions as $permission)
                      <div class="field">
                        <b-checkbox v-model="permissionsSelected" :native-value="{{$permission->id}}">{{$permission->display_name}} <em>({{$permission->description}})</em></b-checkbox>
                            {{-- we need to add the : before the native-value because of the way vue works.  Native-value only works with strings.  
                            If we add a : in the beginning of it, it processes it as an actual method and it will process what is in the quotes
                            instead of assuming it is a string.  Now when we convert this back over to to PHP, it will be an integer as well. --}}
                    </div>
                    @endforeach
                </div>
              </div>
